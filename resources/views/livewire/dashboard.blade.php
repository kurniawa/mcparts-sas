        ...
        <div class="grow ml-16">
            ...
            <form wire:submit.prevent="updateNama" x-data="{open:false}" class="sm:w-full p-5 border rounded border-slate-300 bg-white shadow-xl max-w-sm">
                ...
                <div class="flex items-center">
                    <div class="w-full">
                        <input type="text" name="nama" id="nama" class="input" wire:model="user.nama">
                        @error('user.nama')
                ...
                <div x-show="open" x-transition x-cloak class="text-right mt-5">
                    <button class="btn-indigo-sm rounded">Confirm</button>
                </div>
            ...
            <form wire:submit.prevent="updateUsername" x-data="{open:false}" class="mt-5 sm:w-full p-5 border rounded border-slate-300 bg-white shadow-xl max-w-sm">
                ...
                <div class="flex items-center">
                    <input type="text" name="username" id="username" class="input" wire:model="user.username">
                    <svg x-on:click="open=!open" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-slate-500 ml-1 hover:cursor-pointer">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                    </svg>
                </div>
            ...
            <form wire:submit.prevent="updatePassword" x-data="{open:false}" class="mt-5 sm:w-full p-5 border rounded border-slate-300 bg-white shadow-xl max-w-sm">
                ...
                <input wire:model="current_password" type="password" name="current_password" id="current_password" class="input" placeholder="Current Password">
                @error('current_password')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <label for="new_password" class="text-slate-600 block mt-2">New Password :</label>
                <input wire:model="new_password" type="password" name="new_password" id="new_password" class="input" placeholder="New Password">
                @error('new_password')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <label for="confirm_new_password" class="text-slate-600 block mt-2">Confirm New Password :</label>
                <input wire:model="confirm_new_password" type="password" name="confirm_new_password" id="confirm_new_password" class="input" placeholder="Confirm New Password">
                ...
                <div class="text-right mt-2">
                    <button class="btn-indigo-sm rounded">Update Password</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="open_choose_photo" x-cloak class="absolute bg-white top-5 left-1/2 -translate-x-1/2 sm:w-4/5 p-5 border rounded shadow-xl">
        ...
        <form wire:submit.prevent="savePhoto" class="mt-3 text-center">
            ...
            <div class="mt-3">
                ...
                <div class="alert-success">{{ session('save_photo_logs') }}</div>
                @endif
                @if (session()->has('delete_photo_logs'))
                <div class="alert-danger">{{ session('delete_photo_logs') }}</div>
                @endif
            </div>
            <div class="mt-3">
                <button class="btn-primary-sm rounded" type="submit">Save Photo</button>
            </div>
        </form>
        <button onclick="confirm('Anda yakin ingin menghapus profile picture ini?') || event.stopImmediatePropagation()" wire:click="deletePhoto" class="btn-danger-sm rounded mt-1">Delete</button>
    </div>
    <script>
